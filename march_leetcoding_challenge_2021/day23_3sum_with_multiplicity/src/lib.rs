...
impl Solution {
    pub fn three_sum_multi(arr: Vec<i32>, target: i32) -> i32 {
        let mut count = vec![vec![0; 101]; arr.len()];

        for i in (0..(arr.len() - 1)).rev() {
            for j in 0..101 {
                count[i][j] = count[i + 1][j];
            }
            count[i][arr[i + 1] as usize] += 1;
        }

        let mut sum = 0;
        for i in 0..(arr.len() - 2) {
            for j in (i + 1)..(arr.len() - 1) {
                let remains = target - arr[i] - arr[j];
                if remains < 0 || 100 < remains {
                    continue;
                }
                sum = (sum + count[j][remains as usize]) % 1_000_000_007;
            }
        }
    ...
    fn test_day23() {
        assert_eq!(
            Solution::three_sum_multi(
                vec![
                    92, 4, 59, 23, 100, 16, 7, 15, 3, 78, 98, 17, 77, 33, 83, 15, 87, 35, 54, 72,
                    ...
                    51, 17, 52, 99, 3, 48, 65, 51, 22, 38, 77, 81, 30, 64, 99, 35, 88, 72, 73, 29,
                    29, 2
                ],
                105
            ),
